namespace HideIt.Test.Middleware;
...
public class SecurityHeadersMiddleware {
  ...
  private const string _defaultCspPolicy =
    "default-src 'self'; " +
  ...
  public async Task InvokeAsync( HttpContext context ) {
    ...
    if ( !context.Response.Headers.ContainsKey( "Content-Security-Policy" ) ) {
      string? cspPolicy = _configuration["SecurityHeaders:ContentSecurityPolicy"];
      if ( string.IsNullOrWhiteSpace( cspPolicy ) ) {
        cspPolicy = _defaultCspPolicy;
      }
